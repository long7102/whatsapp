		...
		<>
			...
			>
				<Avatar className='border border-gray-900 overflow-visible relative'>
					...
					<AvatarImage src={conversationImage || me?.image} className='object-cover rounded-full' />
					<AvatarFallback>
				...
				<div className='w-full'>
					<div className='flex items-center'>
						<h3 className='text-xs lg:text-sm font-medium'>{conversationName ? conversationName : me?.name}</h3>
					...
					<p className='text-[12px] mt-1 text-gray-500 flex items-center gap-1 '>
						...
						{!lastMessage && "Bắt đầu câu chuyện!"}
						...
						{lastMessageType === "image" && (
							<span className="flex gap-1">
								Đã gửi 1 hình ảnh <ImageIcon size={16} />
							</span>
						)}
						{lastMessageType === "video" && (
							<span className="flex gap-1">
								Đã gửi 1 video <VideoIcon size={16} />
							</span>
						)}
					</p>
				</div>
