	...
	return (
		<Dialog>
			<DialogTrigger asChild>
				<div className='text-xs text-muted-foreground text-left cursor-pointer'>Xem thành viên</div>
			</DialogTrigger>
			<DialogContent>
				<DialogHeader>
					<DialogTitle className='my-2'>Thành viên hiện tại</DialogTitle>
					<DialogDescription>
								...
								<div key={user._id} className={`flex gap-3 items-center p-2 rounded`}>
									...
									<div className='w-full '>
										<div className='flex items-center gap-2'>
											...
											{user._id === selectedConversation.admin && <Crown size={16} className='text-yellow-500' />}
										</div>
									</div>
								</div>
							))}
