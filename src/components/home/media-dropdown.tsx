...
const MediaDropdown = () => {
	...
	const handleSendImage = async () => {
		try {
			//tạo link để upload
			const postUrl = await generateUploadUrl()
			//đẩy file lên đường dẫn
			...
			toast.error("Có lỗi xảy ra khi gửi file")
		} finally {
			setIsLoading(false)
		}
	}
...
{selectedImage && (
            ...
            <DropdownMenu>
                <DropdownMenuTrigger>
                    <Plus className="text-gray dark:text-gray-400" />
                </DropdownMenuTrigger>
                <DropdownMenuContent>
                    <DropdownMenuItem onClick={() => imageInput.current!.click()}>
                        <ImageIcon size={18} className="mr-1" />Ảnh
                    </DropdownMenuItem>
